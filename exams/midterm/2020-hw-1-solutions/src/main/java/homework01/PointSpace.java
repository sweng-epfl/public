...
public final class PointSpace {
    ...
    public EuclideanVector geometricCenter() {
        ensureNotEmpty();

        // get() is safe, since reduce() never returns an empty Optional if the calling stream is non-empty
        EuclideanVector sum = points.stream().reduce(EuclideanVector::plus).get();
        return sum.scalarDivide(points.size()).plus(origin);
    }
