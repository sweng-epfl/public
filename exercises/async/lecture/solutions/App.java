...
public class App {
    ...
    public static void main(String[] args) {
        ...
        getExactAnswer()
                .thenApply(i -> "Exact: " + i)
                .acceptEither(getApproximateAnswer().thenApply(i -> "Approximate: " + i), System.out::println);
        ...
        getAnswerWithJustification()
                .exceptionallyCompose(e -> getExactAnswer().thenApply(i -> "Exact: " + i))
                .thenAccept(System.out::println);
        ...
        countSheep(flag).orTimeout(5, TimeUnit.SECONDS).exceptionally(e -> {
            flag.set(true);
            return -1;
        }).thenApply(c -> "Sheep count: " + c).thenAccept(System.out::println);
        ...
        firstSuccess(App::unreliableDownload).thenAccept(System.out::println);
        ...
        newAsyncSend("ping").thenAccept(System.out::println);
    ...
    static <T> CompletableFuture<T> firstSuccess(Supplier<CompletableFuture<T>> supplier) {
        return supplier.get().exceptionallyCompose(e -> firstSuccess(supplier));
    }
    ...
    static CompletableFuture<String> newAsyncSend(String text) {
        ...
        oldAsyncSend(text, future::complete, future::completeExceptionally);
        return future;
    }
    ...
    static CompletableFuture<Void> printAnswers(Consumer<String> printer) {
        return CompletableFuture.allOf(
                getExactAnswer()
                        .thenApply(a -> "Exact: " + a)
                        .thenAccept(printer),
                getApproximateAnswer()
                        .thenApply(a -> "Approx: " + a)
                        .thenAccept(printer)
        );
    }
